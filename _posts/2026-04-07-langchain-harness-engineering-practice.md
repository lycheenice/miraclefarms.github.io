...
更关键的是，LangChain 还实现了一个 **`PreCompletionChecklistMiddleware`**，在 agent 退出前强制触发一次验证流程——这类似 [Ralph Wiggum Loop](https://ghuntley.com/loop/) 的思路[[1]](https://blog.langchain.com/improving-deep-agents-with-harness-engineering/)：一个 hook 强制 agent 在"退出"时继续执行，而不是直接返回。
...
LangChain 将"分析 trace"本身也做成了一个 **Agent Skill**[[1]](https://blog.langchain.com/improving-deep-agents-with-harness-engineering/)——这让它可以重复使用。
